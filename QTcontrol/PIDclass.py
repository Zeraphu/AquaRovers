...
pid = PID()  # initializing

output = 0
pid.setoutstart(output)  # set output value

sp = 10
pid.setpoint(sp)  # set setpoint value
print(repr(pid))
if output < sp:
    pid.setKi(0.01)  # setting weight factors
    pid.setKd(0.6)
    pid.setKp(0.2)
    while output < sp:
        output = pid.update(output)  # calling pid to update
